...
%   date: 2023.6.12
    ...
    obj_old = norm(y_residue_matrix, 'fro');
...
%     atom_vec_est_matrix = reshape(atom_vec_est, Nx, My);
    ...
    grad_S = zeros(2, 1);
    for t = 1 : T
        % y_matrix_add(:,:,t) = y_matrix(:,:,t) + ghat(t) * xhat_vec * ...
        %     (yhat_vec.');
        y_matrix_add_temp = y_matrix(:, :, t) + ghat(t) * xhat_vec * ...
            (yhat_vec.');
        y_matrix_add(:, :, t) = y_matrix_add_temp;
        % y_vector_add_temp = y_matrix_add_temp(:);
        % grad_S = grad_S - 2 * real(conj(ghat(t)) * da_over_domega' * ...
        % y_vector_add_temp);
        % [x1diff_vec' * y_matrix_add_temp * conj(yhat_vec); xhat_vec' * y_matrix_add_temp * conj(y1diff_vec)]
        grad_S = grad_S - 2 * real(ghat(t)' * [x1diff_vec' * ...
            y_matrix_add_temp * conj(yhat_vec); xhat_vec' * y_matrix_add_temp...
            * conj(y1diff_vec)]);
        Hessian_S(1, 1) = Hessian_S(1, 1)- 2 * real(ghat(t)' * x2diff_vec' * ...
            y_matrix_add_temp * conj(yhat_vec));
        Hessian_S(2, 2) = Hessian_S(2, 2)- 2 * real(ghat(t)' * xhat_vec' * ...
            y_matrix_add_temp * conj(y2diff_vec));
        Hessian_S(1, 2) = Hessian_S(1, 2) - 2 * real(ghat(t)' * x1diff_vec'...
            * y_matrix_add_temp * conj(y1diff_vec));
...
%     y_matrix_add = y_matrix + ghat * atom_vec_est_matrix;
    ...
    for t = 1 : T
        ...
        y_matrix(:, :, t) = y_matrix_add(:, :, t) - ghat_new(t) * ...
            xhat_vec_new * (yhat_vec_new.');
    end

%     atom_vec_est_new = kron(yhat_vec_new, xhat_vec_new);
%     ghat_new = atom_vec_est_new' * y_vector_add;

    obj_new = norm(y_matrix, 'fro');
    if obj_new < obj_old
        y_residue_matrix = y_matrix;
