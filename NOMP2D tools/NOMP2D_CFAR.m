...
%   此处显示详细说明
    ...
    while (K_est < K_max)
        ...
        [omega_est, ~, ~] = RefineAll_2D(y_residue_matrix, ...
            omegaList(1 : K_est, :), gainList(1 : K_est, :), R_s, R_c);
    ...
    while true
        ...
        if Khat > 1
            for kidx = Khat : -1 : 1
                ...
                [gainList_temp, y_test, ~] = LeastSquares_2D(y_matrix, ...
                    omegaList_temp);
                y_r_det(:, :, :, kidx) = y_test;

                prod_test = prod2D_cal(y_test, overSamplingRate);
                [T_judgement, Threshold_CUT] = CFAR_detector2D(prod_test, ...
                    N_r, alpha_set, omegaList, guard_band);
            ...
            pmat_test = prod2D_cal(ymat_test, overSamplingRate);
            [T_judgement, Threshold_CUT] = CFAR_detector2D(pmat_test, ...
                N_r, alpha_set, omegaList, guard_band);
        ...
        if Tarray_judgement(ktar_idx) < 0
            ...
            pmat_test = prod2D_cal(y_residue_matrix, overSamplingRate);
            [T_judgement, ~] = CFAR_detector2D(pmat_test, N_r, alpha_set, ...
