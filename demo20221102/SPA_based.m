...
function [ospa_vals,mytimer1] = SPA_based(draw_or_not,c,p,scenario)
% clc; 
% clear; 
% close all; 
% rng(24);
...
n_real = zeros(1,100);
  ...
  target_type_indicator = zeros(1,100);
dim_x = 4;
dim_z = 2;
...
ospa_vals_all = zeros(SampleTime,3);
...
%      hold on;
% for k=1:n_max
%   plot(X{k}(1,appeartime(k):disappeartime(k)),X{k}(3,appeartime(k):disappeartime(k)),'b--','LineWidth',2);    %真实值
% end    
%  
%         test_t=Z_Matrix_backup{i};
%     labels = dbscan(Z_Matrix_backup{i},epsilon,minpts);
...
%       assocaite_w_unnomarlized{i} = [assocaite_w_unnomarlized{i} ; a];
        end
        ...
        m =  size(Z_Matrix{i},2);
        om = ones(1,m);
        on = ones(1,n_predict(i));
        muba = ones(m,n_predict(i));
        delta = ones(m,n_predict(i));    %变化量的表示
        interate(Monte_Carlo,i) = 1;
        while(max(max(abs(delta))) > 1e-7)
            muba_pre = muba;
            ...
            delta = muba - muba_pre;
            interate(Monte_Carlo,i) = interate(Monte_Carlo,i) + 1;
        end

        association_p_after = muba.* assocaite_w_unnomarlized{i};      %消息传递后的association
...
%         association_p_after = assocaite_w_unnomarlized{i};          %消息传递前的association
        ...
        for repeat = exist_index             % 先求每个目标的单个PDA
            %消失判定+最有可能测量判定
            [~,dist_indicator{repeat},index] = dist_measure(R, H, ...
                P_Pre{repeat}, X_Pre{repeat}, Z_Matrix{i}, gamma);
            if (isempty(index) == 1)
            ...
            most_likely_measurement = find(association_p_after(:,temp_i) == max(association_p_after(:,temp_i)));
            if(most_likely_measurement<=num_cloud)
                target_type_indicator(repeat) = target_type_indicator(repeat)+1;
            else
            target_type_indicator(repeat) = target_type_indicator(repeat)-1;
              end
             end 
       ...
       [~,temp_index]=sort(Z_chosen);
        ...
        temp_index = temp_index(1:size(temp_index,1)-n_predict(i),1);
        %记录可能的真实测量
              ...
              clear association_p_after_normal;
                  ...
                  association_p_after_normal_temp = [association_p_after_normal'; association_p_after_normal'];
                   ...
                   if ismember(repeat,out_of_circle)
                    ...
                    Xkk{exist_index(repeat)}=Xk_after;
                    Pkk{exist_index(repeat)}=Pk_after;
                    % 预测 
...
%           disappeartime_predict= cat(2,disappeartime_predict ,i);
 ...
 ospa_vals= zeros(SampleTime,3);

for k=1:SampleTime
    [ospa_vals(k,1), ospa_vals(k,2), ospa_vals(k,3)]= ospa_dist(truth{k}([1,3],:),est{k}([1,3],:),ospa_c,ospa_p);
end
ospa_vals_all = ospa_vals_all + ospa_vals;
...
% 
% for k=1:n_max
%     i=appeartime(k):disappeartime(k); 
...
%     i=appeartime_predict(k):disappeartime_predict(k);
...
ospa_vals = ospa_vals_all/M;    
if draw_or_not ~=0    
%   画动图 
pic_num = 1;
figure();
title('Target trajectory(SPA based)','Fontsize',12); xlabel('x/m','Fontsize',12); ylabel('y/m','Fontsize',12);  

box on; hold on;
axis equal;
p1_exist=0;p2_exist=0;p3_exist=0;p4_exist=0;
...
for i=1:SampleTime
 ...
 for k=1:n_max
        if (i>=appeartime(k))&&(i<=disappeartime(k))
            if (target_type(k)~=0)
            p1 = plot(X{k}(1,appeartime(k):i),X{k}(3,appeartime(k):i),'b--','LineWidth',2,'Marker','s','MarkerIndices',1:20:length(appeartime(k):i),'MarkerSize',6);    %真实值
            p1_exist = 1;
            else
            p2 = plot(X{k}(1,appeartime(k):i),X{k}(3,appeartime(k):i),'b--','LineWidth',2,'Marker','o','MarkerIndices',1:20:length(appeartime(k):i),'MarkerSize',6);    %真实值
            p2_exist = 1;
            end
...
for k=1:n_predict_max  
         if (i>=appeartime_predict(k))&&(i<=disappeartime_predict(k))
             if (target_type_indicator(k)>0)
            p3 = plot(Ex_PDA{k}(1,appeartime_predict(k):i),Ey_PDA{k}(1,appeartime_predict(k):i),'r-','LineWidth',2,'Marker','s','MarkerIndices',1:20:length(appeartime_predict(k):i),'MarkerSize',6);    %滤波值
%              plot(Z_PDA{k}(1,i),Z_PDA{k}(2,i),'o','Color', [0.5 0.5 0.5]);       %组合测量值 
             p3_exist = 1;
             else
            p4 = plot(Ex_PDA{k}(1,appeartime_predict(k):i),Ey_PDA{k}(1,appeartime_predict(k):i),'r-','LineWidth',2,'Marker','o','MarkerIndices',1:20:length(appeartime_predict(k):i),'MarkerSize',6);    %滤波值   
             p4_exist = 1;
             end
...
if(p1_exist)
    plot_handle(1,1) = p1;
    name{1,1} = 'truth tracks(extended target)';
    if(p2_exist)
    plot_handle = [plot_handle, p2];
    name = [name,'truth tracks(point target)'];
    end
else
    plot_handle(1,1) = p2;
    name = {'truth tracks(point target)'};
end


if(p3_exist)
    plot_handle = [plot_handle, p3];
    name = [name,'estimated tracks(extended target)'];
end
if(p4_exist)
    plot_handle = [plot_handle, p4];
    name = [name,'estimated tracks(point target)'];
end


  legend(plot_handle,name);
...
%     
  ...
  figure();
    ...
    title(' Number of message passing iterations'); xlabel('t/s'); ylabel('n/times'); 
     set(gca,'YLim',[0,15]);   
...
subplot(3,1,1); plot(1:SampleTime,ospa_vals(:,1),'k'); grid on; set(gca, 'XLim',[1 SampleTime]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Dist');
subplot(3,1,2); plot(1:SampleTime,ospa_vals(:,2),'k'); grid on; set(gca, 'XLim',[1 SampleTime]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Loc');
subplot(3,1,3); plot(1:SampleTime,ospa_vals(:,3),'k'); grid on; set(gca, 'XLim',[1 SampleTime]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Card');
xlabel('Time');

save('ospa_vals.mat','ospa_vals');
