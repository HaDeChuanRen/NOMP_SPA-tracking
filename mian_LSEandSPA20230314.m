...
NM = Nx * My;
N_r = 60;
alpha_2D = alpha_PoebyS(P_fa, NM, N_r);
K_max = 16;
...
xPosseq_hat = cell(sample_time, 1);
yPosseq_hat = cell(sample_time, 1);
xVelseq_hat = cell(sample_time, 1);
yVelseq_hat = cell(sample_time, 1);
...
Kseq_hat(1) = Khat_1;
xPosseq_hat{1} = Zarray_1(:, 1);
yPosseq_hat{1} = Zarray_1(:, 2);
xVelseq_hat{1} = zeros(Khat_1, 1);
yVelseq_hat{1} = zeros(Khat_1, 1);
...
for t_time = 2 : sample_time
    ...
    Khat_t = Kseq_hat(t_time - 1);
    [Promat_a, Promat_b] = SPA_DA(XPri_t, Zxy_t, Hmat_mea, Smat_t, Pd_xki, ...
        mu_c, L_ite);
    ...
    XPost_t = zeros(state_dim, Khat_t);
    % the sequence of the variable is : p_x, v_x, p_x, v_y
    for k_tidx = 1 : Khat_t
        [~, aPrivec_kt] = max(Promat_a(:, k_tidx));
...
for t_time = 1 : sample_time
    ...
    phandle_hat = plot(xPosseq_hat{t_time}, yPosseq_hat{t_time}, 'r*', ...
        'LineWidth', Lw, 'Marker', 's', 'MarkerSize', Msz);
    xlim([-120 120])
    ylim([0 120])
    pause(0.1);
    % for k_kidx = 1 : K_targets
    ...
    % for k_kidx = 1 : Kseq_hat(t_time)
    %     phandle_hat = plot(xPosseq_hat{t_time}(k_idx), ...
    %         yPosseq_hat{t_time}(k_idx), 'r', 'LineWidth', Lw, ...
    %         'Marker', 's', 'MarkerSize', Msz);
    %     hold on;
    % end
