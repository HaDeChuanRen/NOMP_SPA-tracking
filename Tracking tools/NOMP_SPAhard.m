function [history_all, label_all, Zcol_cell] = NOMP_SPAhard(yten_spec, alpha_2D, ...
    N_r, K_max, rmax, vmax, Pmat_cov, Qmat_noise, Amat_state, Hmat_mea, ...
    Rmat_noise)
% last update: 2023/7/24
% 2023/7/24 update:
% 1. (Todo) revise and add comments
% 2. (Todo) use the appear state flag to decide which target has disappeared.
    ...
    elseif Mhat_t > 0 && Khat_last > 0
        ...
        for k_idx = 1 : Khat_last
            ...
            if aPrivec_kt <= Mhat_t
                ...
                %     Hmat_mea' + Rmat_noise);
                target_cell(k_idx) = target_cell(k_idx).est_hard(Rmat_noise, ...
                    Hmat_mea, ZPost_kt);
            elseif aPrivec_kt == Mhat_t + 1
                target_cell(k_idx) = target_cell(k_idx).disappearing();
                if target_cell(k_idx).P_det < 0.5
                    disapp_set = [disapp_set, k_idx];
                end
            end
        end
