function [i, j] = Hungarianmatch(cost_matrix)
...
s = struct('C', cost_matrix, 'row_uncovered', true(n, 1),  'col_uncovered', ...
    true(1, m), 'Z0_r', 1, 'Z0_c', 1, 'path', ones(n+m, 2, 'int64'), ...
...
state.C = state.C - repmat(min(state.C,[],2), [1, size(state.C, 2)]);
...
if any(state.row_uncovered) & any(state.col_uncovered)
    minval = min(min(state.C(state.row_uncovered, state.col_uncovered)));
    state.C(~state.row_uncovered, :) =  state.C(~state.row_uncovered, :) ...
        + minval;
    state.C(:, state.col_uncovered) = state.C(:, state.col_uncovered) - minval;
end
