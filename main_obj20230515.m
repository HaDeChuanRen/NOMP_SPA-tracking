...
T = 1;    % sample interval
sigma_w = [0.01; 0; 0.01; 0];    % state transition noise variance
...
for t_time = 2 : sample_time
    ...
    if Kseq_hat(t_time - 1) == 0 && Mhat_t > 0
        statevec_k = [Zarray_t(1, 1), 0, Zarray_t(1, 2), 0]';
        targetobj_hat = TargetState(statevec_k, Pmat_cov, Qmat_noise, ...
            Amat_state, t_time, T);
        Kseq_hat(t_time) = Mhat_t;
    elseif Kseq_hat(t_time - 1) > 0 && Mhat_t > 0
        ...
        Kmat_gain = Pmat_pri * Hmat_mea' / (Hmat_mea * Pmat_pri * ...
            Hmat_mea' + Rmat_noise);
        targetobj_hat = targetobj_hat.state_estimate(Smat_t, Hmat_mea, ...
            Zxy_t(z_idx, :));
        Kseq_hat(t_time) = Mhat_t;
    elseif Kseq_hat(t_time - 1) == 1 && Mhat_t == 0
        [XPri_t, Pmat_pri, targetobj_hat] = targetobj_hat.state_transform();
        targetobj_hat = targetobj_hat.disappearing();
    end
...
for t_time = 1 : sample_time - 1
    ...
    plot(xlim_vec, ybon_vec, '--k', 'LineWidth', Lw);
