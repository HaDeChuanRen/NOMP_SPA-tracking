...
omega_z = sin(theta_set) * pi;

% omega_x = 1 : 4;
% omega_y = 5 : 8;

omega_true = wrapTo2Pi([omega_x; omega_y; omega_z]);
...
% tic;
% [omega_low, gain_low, ~, ~] = NOMP2D_low(y_matrix, alpha_hat, N_r, K_max);
% omega_low = wrapToPi(omega_low);
% toc;
%
% S_snap = 1;
...
y_vec = zeros(NML, 1);
for k_idx = 1 : K_targets
    y_veck = kron(kron(array_Fun(omega_z(k_idx), Lz), ...
        array_Fun(omega_y(k_idx), My)), array_Fun(omega_x(k_idx), Nx));
    y_vec = gain_allK(k_idx) * y_veck + y_vec;
end
y_ten = reshape(y_vec, [Nx, My, Lz]) + sqrt(sigma_n / 2) * ...
    (randn(Nx, My, Lz) + 1j * randn(Nx, My, Lz));

alpha_hat = alpha_PoebyS(p_oe, NM_num, N_r, Lz);
tau = sigma_n * chi2inv((1 - p_oe) ^ (1 / NML), 2 * Lz) / 2;
% [omegaList, gainList, ~] = MNOMP2D(y_ten, tau);

tic;
[omega_low, gain_low, ~, ~] = MNOMP2D_fast(y_ten, alpha_hat, N_r, K_max);
toc;

gamma_l = 128;
yvec_zabs = abs(fft(gain_low, gamma_l, 2));
[~, z_idx] = max(yvec_zabs, [], 2);
omegazhat_t = (z_idx / gamma_l) * 2 * pi;
